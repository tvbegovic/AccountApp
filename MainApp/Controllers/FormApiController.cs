...
namespace AccountApplication.Controllers
{
    ...
    public class OnlineFormController : ControllerBase
    {
	    ...
	    private ILoqateServicesClient loqateServicesClient;
	    public const string ValidationRequiredMessage = "Field is required.";


	    public OnlineFormController(IUnitOfWork unitOfWork, IHostingEnvironment hostingEnvironment,
		    IConfiguration configuration, ILoqateServicesClient loqateServicesClient)
	    {
		    this.loqateServicesClient = loqateServicesClient;
	    ...
	    [HttpGet("addressSearch/{postcode}")]
	    public List<string> GetAddressMatches(string postcode)
	    {
		    var url = _configuration.GetValue<string>("appSettings:addressSearch:url");
		    var key = _configuration.GetValue<string>("appSettings:addressSearch:key");
		    return loqateServicesClient.GetAddressMatches(url, key, postcode);
		...
		//private object GetUIObject(Form form, Dictionary<int?, Form_submission_answer> answers = null, List<Form_question_type> types = null)
  //      {
		//	if (form != null)
  //              return new
  //              {
  ...
  //                  sections = form.Sections?.OrderBy(s=>s.sequence).Select(s=> new {
		...
		//					s.Section?.Id,
		//					s.Section?.title,
		//					questions = s.Section?.Questions?.OrderBy(q=>q.sequence)?.Select(q=> new {
		//						q.id,
		//						q.question_group_id,
		//						q.question_id,
		//						section_id = s.Section?.Id,
		//						question = q.Question != null ?  GetQuestionUIObject(q.Question, types) : null,
		//						questionGroup = q.QuestionGroup != null ? new {
		//							q.QuestionGroup.id,
		//							q.QuestionGroup.name,
		//							q.QuestionGroup.description,                                
		//							questions =  ProcessQuestionGroupQuestions(q.QuestionGroup?.Questions, types)
		//						} : null,
		//						answer = answers != null && answers.ContainsKey(q.id) ?
		//							answers[q.id] :
		//							new Form_submission_answer
		//							{
		//								formsection_question_id = q.id,
		//								QuestionAnswers = q.Question != null ? new List<form_question_answer>
		//								{
		//									new form_question_answer
		//									{
		//										question_id = q.Question.id
		//									}
		//								} : 
		//								q.QuestionGroup?.Questions.OrderBy(x=>x.sequence).Select(x => new form_question_answer
		//								{
  ...
  //      }
	    ...
	    //private object ProcessQuestionGroupQuestions(List<Form_questiongroup_question> questions, List<Form_question_type> types = null)
	    //{
		   ...
		   // {
			  ...
			  //  {
				 //   result.Add(new {question = GetQuestionUIObject(item.Question, types)});
			  //  }
		   // }
		   // return result;
	    //}

	   // private object GetQuestionUIObject(Form_question q, List<Form_question_type> types = null)
	   // {
		  //  return new
		  //  {
			 //   q.id,
			 ...
			 //   q.description,
			 //   q.question_type,
			 //   render_id = q.render_id ?? types?.FirstOrDefault(t=>t.id == q.question_type)?.default_render_id,
			 //   q.color,
			 //   q.comment_label,
			 //   q.choice_group_id,
			 //   q.label_editable,
